...
async function PayloadLoader(Pfile) {
  ...
  try {
    ...
    //window.log(`PayloadLoader error: ${e}`);
    return 0;
  }
  return 1;
}
...
function checkPlatformIsSupported() {
  ...
  src_part = res;
  window.log("Detected FW: PS" + device + " v" + fwVersion + ", Exploit Version: v1.5.2\n");
  // Supported FW lists
  var supportedFW = {
...
async function doJBwithPSFreeLapseExploit() {
  ...
  try {
    ...
    window.log('Lapse STAGE 2/5: Leak kernel addresses');
    window.log('If stuck 10 seconds, restart your console');
    await sleep(50); // Wait 50ms
